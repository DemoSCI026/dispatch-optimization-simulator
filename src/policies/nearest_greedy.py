"""Nearest-distance baseline policy."""
...
class NearestGreedyPolicy(Policy):
    """Assign each order to the courier closest to its pickup point."""
    ...
    def solve(
        ...
        for order in state.candidate_orders:
            ...
            for courier in iter_eligible_couriers(state):
                pickup_distance = cost_engine.travel_distance(
                    courier.lat,
                    courier.lng,
                    order.pickup_lat,
                    order.pickup_lng,
