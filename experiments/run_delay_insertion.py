"""Run the delay-insertion dispatch heuristic."""
...
from src.policies.delay_insertion import DelayInsertionPolicy
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the delay-insertion dispatch heuristic.")
    parser.add_argument(
        "--data-dir",
        type=str,
        default=None,
    ...
    parser.add_argument(
        "--preset",
        choices=tuple(DETOUR_PRESETS),
        default="mid",
        help="Delay-threshold preset for detour gating.",
    )
    parser.add_argument(
        "--strict-real-data",
        action="store_true",
    ...
    detour_threshold = DETOUR_PRESETS[args.preset]
    policy = DelayInsertionPolicy(detour_threshold=detour_threshold)
    ...
    print(f"preset={args.preset} detour_threshold={detour_threshold:.1f}")
